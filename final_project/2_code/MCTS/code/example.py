...
pp.infotext = 'name="modified_MCTS", author="Yida", version="1.0"'
...
def brain_my(x, y):
    if isFree(x, y):
        board[x][y] = 1
        player.method.brain_update((x, y))
    else:
    ...
    if count_num is None:
        ...
        for i in range(len(board)):
            for j in range(len(board)):
                if board[i][j] == 1:
                    count_num += 1
    time_limit = 5 + time_start if count_num > 3 else -1
    (x, y) = player.action(board, time_limit)
